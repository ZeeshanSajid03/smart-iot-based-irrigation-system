...
import { FaPlus, FaCamera, FaLock, FaTimes, FaPen, FaSave, FaEnvelope, FaPhone, FaUserShield } from 'react-icons/fa';
import { Modal } from 'react-bootstrap'; 

const AdminProfilePage = () => {
  const fileInputRef = useRef(null);
  
  const [userData, setUserData] = useState({ photoUrl: null, firstName: '', lastName: '', username: '', email: '', phone: '' });
  ...
  useEffect(() => {
    ...
    if (storedUser) {
        ...
        setUserData({ ...parsedUser, phone: parsedUser.phone || '' });
    }
  }, []);
  ...
  const handleFieldSave = async (fieldName, newValue) => {
    try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/update-profile`, {
            email: identifierEmail, fieldName: fieldName, newValue: newValue
        });
  ...
  const requestSensitiveChange = async (fieldName, newValue, password) => {
      try {
          const res = await axios.post(`${import.meta.env.VITE_API_URL}/request-sensitive-change`, {
              email: userData.email, fieldName: fieldName, newValue: newValue, password: password
          });
  ...
  const handleVerifyOtp = async () => {
      try {
          const res = await axios.post(`${import.meta.env.VITE_API_URL}/verify-sensitive-change`, { email: userData.email, otp: otp });
  ...
  const handleEmailSaveClick = async () => {
      if (!emailAuthPassword) return alert("Password is required for security.");
  ...
  return (
    <div className="container mt-5">
      <div className="d-flex align-items-center justify-content-center gap-2 mb-4">
          <FaUserShield size={28} className="text-secondary" />
          <h2 className="text-secondary fw-bold mb-0">Administrator Profile</h2>
      </div>
      ...
      <div className="profile-card rounded-5 p-5 mx-auto d-flex flex-column align-items-center text-center position-relative shadow" style={{ maxWidth: '900px', backgroundColor: '#3d7b65' }}>
          ...
          <div className="mb-4 position-relative">
              <div 
                  className="rounded-circle d-flex align-items-center justify-content-center bg-secondary bg-opacity-25 clickable-image border border-3 border-light shadow"
          ...
          <div className="w-100" style={{ maxWidth: '600px' }}>
              ...
              <div className="mb-4 text-start">
                  <label className="text-white small fw-bold ms-2">Admin Username</label>
              ...
              <h5 className="text-white text-start fw-bold mb-3">Security & Contact Details</h5>
                  ...
                  {!isEditingEmail ? (
                      <div className="d-flex align-items-center justify-content-between p-3 rounded-3 text-white bg-dark bg-opacity-25 border border-white border-opacity-10">
                          <div className="text-truncate">
                              <span className="fw-bold me-2">Admin Email:</span>
                              <span>{userData.email}</span>
                          </div>
                          <FaPen style={{ cursor: 'pointer' }} className="text-white opacity-75 hover-opacity-100" onClick={() => { setTempEmail(userData.email); setIsEditingEmail(true); }} />
                      </div>
                  ) : (
                      <div className="p-3 rounded-3 bg-white shadow-sm text-dark">
                          <div className="mb-3">
                              <label className="small fw-bold text-secondary mb-1">New Admin Email</label>
                              <div className="input-group">
                                  <span className="input-group-text bg-light border-end-0"><FaEnvelope className="text-muted"/></span>
                                  <input type="email" className="form-control border-start-0 ps-1" value={tempEmail} onChange={(e) => setTempEmail(e.target.value)} />
                              </div>
                          </div>
                          <div className="mb-3">
                              <label className="small fw-bold text-danger mb-1">Authorization Required</label>
                              <div className="input-group">
                                  <span className="input-group-text bg-light border-end-0"><FaLock className="text-muted"/></span>
                                  <input type="password" className="form-control border-start-0 ps-1" placeholder="Enter current password" value={emailAuthPassword} onChange={(e) => setEmailAuthPassword(e.target.value)} />
                              </div>
                          </div>
                          <div className="d-flex gap-2 justify-content-end">
                              <button className="btn btn-sm btn-light text-danger fw-bold" onClick={() => { setIsEditingEmail(false); setEmailAuthPassword(""); }}><FaTimes /> Cancel</button>
                              <button className="btn btn-sm btn-success fw-bold px-3" onClick={handleEmailSaveClick}><FaSave /> Save</button>
                          </div>
                      </div>
                  )}
              </div>
              ...
              <div className="mb-3 text-start">
                  {!isEditingPhone ? (
                      <div className="d-flex align-items-center justify-content-between p-3 rounded-3 text-white bg-dark bg-opacity-25 border border-white border-opacity-10">
                          <div className="text-truncate">
                              <span className="fw-bold me-2">Emergency Phone:</span>
                              <span>{userData.phone || "Not Set"}</span>
                          </div>
                          <FaPen style={{ cursor: 'pointer' }} className="text-white opacity-75 hover-opacity-100" onClick={() => { setTempPhone(userData.phone); setIsEditingPhone(true); }} />
                      </div>
                  ) : (
                      <div className="p-3 rounded-3 bg-white shadow-sm text-dark">
                          <div className="mb-3">
                              ...
                              <div className="input-group">
                                  ...
                                  <input type="text" className="form-control border-start-0 ps-1" value={tempPhone} onChange={(e) => setTempPhone(e.target.value)} />
                              </div>
                          </div>
                          <div className="d-flex gap-2 justify-content-end">
                              <button className="btn btn-sm btn-light text-danger fw-bold" onClick={() => setIsEditingPhone(false)}><FaTimes /> Cancel</button>
                              <button className="btn btn-sm btn-success fw-bold px-3" onClick={handlePhoneSaveClick}><FaSave /> Save</button>
                          </div>
                      </div>
                  )}
              </div>
              ...
              <div className="mt-4">
                  <button className="btn btn-warning w-100 py-3 fw-bold rounded-3 shadow-sm d-flex justify-content-center align-items-center gap-2" onClick={() => setShowPasswordModal(true)}>
                      <FaLock /> Update Admin Password
                  </button>
              </div>
            ...
            <div className="modal-content rounded-4 border-0">
              ...
              <div className="modal-body p-4">
                <form onSubmit={(e) => {
                    ...
                    <div className="mb-3">
                        <input type="password" className="form-control bg-light" required placeholder="Current Password" value={passData.oldPassword} onChange={(e) => setPassData({...passData, oldPassword: e.target.value})} />
                    </div>
                    <div className="mb-3">
                        <input type="password" className="form-control bg-light" required placeholder="New Password" value={passData.newPassword} onChange={(e) => setPassData({...passData, newPassword: e.target.value})} />
                    </div>
                    <div className="mb-3">
                        <input type="password" className="form-control bg-light" required placeholder="Confirm New Password" value={passData.confirmPassword} onChange={(e) => setPassData({...passData, confirmPassword: e.target.value})} />
                    </div>
      ...
      <Modal show={showOtpModal} onHide={() => setShowOtpModal(false)} centered>
        ...
        <Modal.Body className="text-center p-4">
            ...
            <input type="text" maxLength="4" className="form-control text-center fw-bold fs-3 letter-spacing-2 mb-4" placeholder="0000" value={otp} onChange={(e) => setOtp(e.target.value)} />
            
            <button onClick={handleVerifyOtp} className="btn btn-success w-100 fw-bold" disabled={timer === 0}>
