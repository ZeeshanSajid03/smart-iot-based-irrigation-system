...
// ── Build chart data from pumpRuntimeHistory + today's live runtime ───────────
const buildWaterData = (pumpRuntimeHistory = [], todayRuntimeSeconds = 0, flowRateLPM = 15) => {
    ...
    return dayLabels.map(d => {
        ...
        // For today: use live runtime. For past: use history (which includes sensor-based runtime)
...
const evaluateAlerts = (reading) => {
    ...
    const alerts  = [];
    const avgSoil = (reading.soil1 + reading.soil2) / 2;
    ...
    if (avgSoil < THRESHOLDS.SOIL_CRITICAL)       alerts.push({ id: 'soil_critical', type: 'critical', icon: 'exclamation', title: `Critical: Soil moisture at ${avgSoil.toFixed(0)}%`, detail: `Below ${THRESHOLDS.SOIL_CRITICAL}% — wilting risk` });
    else if (avgSoil < THRESHOLDS.SOIL_WARNING)   alerts.push({ id: 'soil_warning',  type: 'warning',  icon: 'exclamation', title: `Low soil moisture: ${avgSoil.toFixed(0)}%`,   detail: `Below ${THRESHOLDS.SOIL_WARNING}% — irrigation recommended` });
    ...
    if (alerts.length === 0) alerts.push({ id: 'all_ok', type: 'ok', icon: 'check', title: 'All systems normal', detail: `Soil ${avgSoil.toFixed(0)}% · Temp ${reading.temperature}°C · Humidity ${reading.humidity}%` });
    return alerts;
};

// ── ANIMATED SENSOR CARD ──────────────────────────────────────────────────────
const SensorCard = ({ label, value, unit, bgColor, borderColor, textColor, prevValue }) => {
    ...
    return (
        <div
            ...
            style={{
                ...
                boxShadow: hovered
                    ? `0 8px 24px ${textColor}33`
                    : flash
                    ? `0 0 0 3px ${textColor}44`
        ...
        >
            ...
            <div style={{
                ...
                background: `radial-gradient(circle at 60% 40%, ${textColor}08, transparent 60%)`,
                opacity: hovered ? 1 : 0,
                transition: 'opacity 0.3s',
                pointerEvents: 'none',
            }} />
            <div className="small text-muted mb-1" style={{ fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {label}
            </div>
            <div style={{
                fontWeight: 800, fontSize: '1.8rem', lineHeight: 1, color: textColor,
                transition: 'all 0.3s',
            ...
            {flash && (
                <div style={{
                    ...
                    animation: 'flashRing 0.6s ease forwards',
                    pointerEvents: 'none',
                }} />
            )}
        </div>
...
const CustomTooltip = ({ active, payload, label, flowRate }) => {
    ...
    return (
        <div style={{
            ...
            padding: '14px 18px', boxShadow: '0 8px 28px rgba(0,0,0,0.12)',
            fontSize: '0.82rem', minWidth: '160px',
        }}>
            <div style={{ fontWeight: 700, color: '#1e293b', marginBottom: '8px', fontSize: '0.92rem' }}>{label}</div>
            <div style={{ color: '#0284c7', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                💧 <strong>{litres} L</strong> used
            </div>
            <div style={{ color: '#64748b', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ⏱ Pump ran ~{runtimeMinutes} min
            </div>
            <div style={{ color: '#94a3b8', fontSize: '0.7rem', marginTop: '6px', borderTop: '1px solid #f1f5f9', paddingTop: '6px' }}>
                Based on {flowRate} L/min flow rate
            </div>
        </div>
    );
};
    ...
    <div style={{
        ...
        <div style={{ position: 'relative', width: '52px', height: '52px', flexShrink: 0 }}>
            <div style={{
                ...
                width: '32px', height: '10px', borderRadius: '50%',
                background: 'rgba(110,231,183,0.25)', animation: 'ripple 1.2s ease-out infinite',
            }} />
            {[{ left: '10px', delay: '0s' }, { left: '24px', delay: '0.4s' }, { left: '38px', delay: '0.8s' }].map((d, i) => (
                <div key={i} style={{
                    position: 'absolute', top: 0, left: d.left,
                    width: '8px', height: '12px',
                    background: i === 1 ? '#34d399' : '#6ee7b7',
        ...
        <div style={{ flex: 1 }}>
            <div style={{ color: '#ecfdf5', fontWeight: 700, fontSize: '0.95rem', marginBottom: '2px', animation: 'irrigPulse 2s ease-in-out infinite' }}>
                💧 Irrigation Active
            </div>
            <div style={{ color: '#6ee7b7', fontSize: '0.75rem' }}>
                Pump is running — water is being delivered to your fields
            </div>
        </div>
        <div style={{
            background: 'rgba(110,231,183,0.2)', border: '1px solid #6ee7b7',
            borderRadius: '20px', padding: '4px 12px', color: '#6ee7b7',
...
const DashboardPage = () => {
    ...
    const maybeSendNotification = async (alert, email) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/notifications/send`, {
                target: email, header: headerMap[alert.id] || 'System Alert',
                message: `${alert.title}. ${alert.detail}`, image: null,
            });
        } catch (err) { /* silent */ }
    };
        ...
        const fetchPumpState = async () => {
            try {
                ...
                if (res.data.status === 'success') {
                    ...
                    setTodayRuntime(runtime);
                    setFlowRate(fr);
                    setPumpHistory(history);
                    flowRateRef.current = fr;
                    historyRef.current  = history;

                    // Rebuild chart — history covers past days, runtime covers today
    ...
    const maxDay      = waterData.length
        ? waterData.reduce((best, d) => d.litres > best.litres ? d : best, waterData[0])
        : { day: '—', litres: 0 };
                ...
                {latestReading && (
                    <div style={{
                        ...
                        <span style={{
                            ...
                            animation: 'irrigPulse 1.5s ease-in-out infinite',
                        }} />
                        Sensors Online
                    </div>
                )}
            ...
            <div className="dash-section mb-4">
                <div
                    style={{
                        ...
                        background: 'linear-gradient(135deg, #e2efeb 0%, #d1fae5 100%)',
                ...
                >
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700, color: '#065f46', fontSize: '0.92rem' }}>
                            <FaThermometerHalf style={{ color: '#10b981' }} />
                    ...
                    <div style={{ padding: '8px 16px 16px' }}>
                        <div className="row g-3">
                            <div className="col-6 col-md-3">
                                <SensorCard
                                    label="Temperature"
                                    value={loadingSensor ? null : latestReading?.temperature ?? null}
                                    unit="°C"
                                    bgColor="#fef3c7"
                                    borderColor="#fcd34d"
                                    textColor="#d97706"
                            ...
                            <div className="col-6 col-md-3">
                                <SensorCard
                                    ...
                                    value={loadingSensor ? null : latestReading?.soil1 ?? null}
                                    unit="%"
                                    bgColor="#a7f3d0"
                                    borderColor="#6ee7b7"
                                    textColor="#059669"
                            ...
                            <div className="col-6 col-md-3">
                                <SensorCard
                                    ...
                                    value={loadingSensor ? null : latestReading?.soil2 ?? null}
                                    unit="%"
                                    bgColor="#d1fae5"
                                    borderColor="#a7f3d0"
                                    textColor="#047857"
                            ...
                            <div className="col-6 col-md-3">
                                <SensorCard
                                    label="Humidity"
                                    value={loadingSensor ? null : latestReading?.humidity ?? null}
                                    unit="%"
                                    bgColor="#bae6fd"
                                    borderColor="#7dd3fc"
                                    textColor="#0284c7"
                ...
                <div className="col-lg-6">
                    <div
                        style={{
                            ...
                            backgroundColor: '#b1c2bc',
                            border: '1px solid #9db4ae',
                    ...
                    >
                        <div style={{
                            ...
                            {systemAlerts.filter(a => a.type !== 'ok').length > 0 && (
                                <span style={{
                                    marginLeft: 'auto', backgroundColor: '#dc2626', color: '#fff',
                                    borderRadius: '20px', padding: '2px 10px', fontSize: '0.68rem', fontWeight: 700,
                                }}>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <div style={{ color: '#6ee7b7', fontSize: '0.75rem', opacity: 0.85 }}>
                                Calculated from pump runtime × {flowRate} L/min — Last 7 days
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }} className="d-none d-md-flex">
                            {[
                                { label: '7-Day Total', value: `${totalLitres} L`, color: '#38bdf8' },
                                { label: 'Daily Avg',   value: `${avgLitres} L`,   color: '#6ee7b7' },
                                { label: 'Peak Day',    value: maxDay.day,          color: '#fbbf24' },
                            ].map((stat, i) => (
                                <div key={i} style={{
                                    backgroundColor: 'rgba(255,255,255,0.1)',
                                    borderRadius: '10px', padding: '8px 14px', textAlign: 'center',
                                    minWidth: '80px', border: '1px solid rgba(255,255,255,0.15)',
                                    transition: 'background 0.2s',
                                }}
                                    onMouseEnter={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.18)'}
                                    onMouseLeave={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.1)'}
                                >
                                    <div style={{ color: stat.color, fontSize: '0.62rem', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2px' }}>
                                        {stat.label}
                                    </div>
                    ...
                    <div style={{ backgroundColor: '#e2efeb', padding: '20px 16px 16px' }}>
                        {totalLitres === 0 ? (
                            <div style={{
                                ...
                                <div style={{ color: '#94a3b8', fontSize: '0.75rem', marginTop: '4px' }}>
                                    Turn the pump on to start tracking water usage.
                                </div>
                            </div>
                        ) : (
                            <ResponsiveContainer width="100%" height={240}>
                                <BarChart data={waterData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barCategoryGap="35%">
                                    ...
                                    <Tooltip
                                        content={<CustomTooltip flowRate={flowRate} />}
                                        cursor={{ fill: 'rgba(16,185,129,0.08)', radius: 8 }}
                                    />
                                    {avgLitres > 0 && (
                                        <ReferenceLine y={avgLitres} stroke="#d97706" strokeDasharray="5 4" strokeWidth={1.5}
                                            label={{ value: `Avg ${avgLitres}L`, position: 'insideTopRight', fill: '#d97706', fontSize: 11, fontWeight: 600 }}
                                        />
                                    )}
                        ...
                        <div style={{
                            ...
                            marginTop: '12px', fontSize: '0.72rem', color: '#4b5563',
                        }}>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                <span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '2px', backgroundColor: '#059669' }} />
                                Today
                            </span>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                            ...
                            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                <span style={{ display: 'inline-block', width: '22px', height: '0px', borderTop: '2px dashed #d97706' }} />
                                Daily average
                            </span>
                            <span style={{ marginLeft: 'auto', color: '#6b7280', fontStyle: 'italic' }}>
                                Estimation based on {flowRate} L/min pump flow rate
                            </span>
                        </div>
                    </div>
                </div>
